        ...
        <div class="card">
            <div class="page_title_block">
                <div class="col-md-5 col-xs-12">
                    <div class="page_title">Edit Card Image</div>
                </div>
            </div>
            <div class="clearfix"></div>
            <div class="card-body mrg_bottom">
                <form action="{{ route('card.update') }}" method="POST" class="form form-horizontal" enctype="multipart/form-data">
                    @csrf
                    <div class="section">
                                ...
                                <div class="col-md-6">
                                    <select name="category_name" id="cat_id" class="select2" required>
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->name }}" {{($category->name == $edit->category_name)? 'selected' : ''}}>{{ $category->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
